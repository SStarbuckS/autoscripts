...
def get_product_info(product_url, expected_price_str):
    ...
    if response.status_code == 200:
        ...
        if current_price and float(current_price) <= float(expected_price_str):
            send_notification(sp_name, product_url, current_price, expected_price_str)
...
def send_notification(sp_name, product_url, current_price, expected_price_str):
        title = "商品低价提醒"
...
# 例子：多个商品链接和预期价格
product_and_prices = [
    {"url": "https://detail.tmall.com/item.htm?id=123", "expected_price": "299.00"},
    # 添加更多商品链接和预期价格...
]
